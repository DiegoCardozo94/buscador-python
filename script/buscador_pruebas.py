...
def hash_parcial(path, max_bytes=MAX_BYTES):
    try:
        ...
        print(f"HASH de {os.path.basename(path)}: {hash_md5.hexdigest()[:8]}...")  # ← Agregado
...
def escanear_y_hash(carpeta, max_hilos=MAX_HILOS, max_archivos=MAX_ARCHIVOS):
    ...
    contador_label.config(text="Archivos escaneados: 0")
    ...
    with ThreadPoolExecutor(max_workers=max_hilos) as executor:
        ...
        for future in as_completed(futures):
            pass  # Solo esperamos a que terminen
    ...
    def actualizar_progreso():
        ...
        while not resultado_queue.empty():
            ...
            contador_label.config(text=f"Archivos escaneados: {contador}/{total}")
        if contador < total:
            ventana.after(50, actualizar_progreso)

    ventana.after(50, actualizar_progreso)

    # <- Esperar hasta que resultado_queue se vacíe completamente
    while not resultado_queue.empty():
        time.sleep(0.05)
        actualizar_progreso()
    ...
    try:
        ...
        nuevo_cache_existentes = {
            k: v for k, v in nuevo_cache.items() if os.path.exists(k)
        }
        with open(cache_path, "w", encoding="utf-8") as f:
            json.dump(nuevo_cache_existentes, f, indent=2)
...
def seleccionar_carpeta():
    ...
    if carpeta:
        ...
        threading.Thread(target=buscar_duplicados, args=(carpeta,), daemon=True).start()
...
def eliminar_duplicados_automatico():
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        futures = {executor.submit(eliminar_archivo, ruta): ruta for ruta in archivos_a_eliminar}
...
contador_label = ttk.Label(ventana, text="Archivos escaneados: 0")
contador_label.pack()
